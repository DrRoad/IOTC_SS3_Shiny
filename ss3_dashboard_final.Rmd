...
### AEN TO DO: Is there a way to have a sidebar for each page whose options are linked to all pages?
##             Add another model run for each spp!! 
##             Add FLEET NAMES (outputs2netcdf)
##             Add SKJ and BFT, grid
##             Figure out ggplotly so that plots become interactive (why does it work for cpue fit and not the rest?)
##             Add short description of each selected run in the sidebar?
##             If models don't have multiple runs, don't allow 'multiple' to be selected

############### DEFINE VRE DIRECTORIES ###############
##     COMMENTED OUT FOR LOCAL USE
# rm(list=ls())
...
# # opts_knit$set(root.dir =(paste(home_dir,SS3_dir,"/IOTC_SS3_Shiny",sep="")))
# setwd(paste(home_dir,SS3_dir,"/IOTC_SS3_Shiny",sep=""))

# dirMain     <- "~/Documents/BLUEBRIDGE/SHINY/shiny_SS3/"
# dirMain <-getwd()
########################################################

############### DEFINE LOCAL DIRECTORIES ###############
#     COMMENT OUT FOR USE ON VRE
dirMain     <- "~/Documents/BLUEBRIDGE/SHINY/shiny_SS3_Git/"
dirRScripts <- paste(dirMain,"Rscripts/",sep='')
########################################################

################### DATA DIRECTORY #####################
#     USE THE THREDDS ADDRESS WHEN DATA ARE UPDATED
# parameter and run/seed info
dirData     <- paste(dirMain, "data/", sep="")
# dirData  <- "https://thredds.d4science.org/thredds/fileServer/public/netcdf/IOTC_SS3_Stock_Assessment_Model_Outputs_Samples/"
########################################################  
...
library(pacman)
p_load(shiny, rCharts, plyr, reshape2, ggplot2, ncdf4, flexdashboard, plotly,googleVis)


# source(paste(dirRScripts,'plot_outputs_SS3.R',sep=''))
run_parameters <- read.csv(paste(dirData, "run_spec_ss3.csv", sep=""))
r_seed <- 1
#######################################################
...
selectizeInput(inputId='Model',label='Select from the species models currently available',
              choices=c('simple','SWO','YFT','BET','SKJ','BFT'),selected='SWO',multiple = FALSE)
...
renderPlot({
    ...
    p+geom_bar(stat="identity")+facet_wrap(~Fleet, ncol=2, scales="free")+
...
renderPlot({
    ...
    p+geom_point()+geom_line(col="black")+geom_line(aes(y=Exp,colour=Model_run),size=1.3)+
      facet_wrap(~Fleet,scales = 'free', ncol=2)#+ 
...
renderPlot({
  ...
  p+geom_line(stat="identity",size=1.3)+facet_wrap(~Area, ncol=2)+ylab('Total Biomass (x1000 t)')+
...
renderPlot({
  ...
  p+geom_line(stat="identity",size=1.3)+ylab('Total Biomass (x1000 t)')+ylim(range(forBAtot$bio))+
     geom_vline(xintercept = end_current_period,col="blue",linetype=2)
  #########################################################################
...
renderPlot({
  ...
  p+geom_line(stat="identity",size=1.3)+facet_wrap(~Area, ncol=2)+ylab('Depletion (%)')+
...
renderPlot({
  ...
  p+geom_line(stat="identity",size=1.3)+ylab('Depletion (%)')+ylim(0,max(forBAtot0$bio/forBAtot0$bio[1]*100))+
     geom_hline(yintercept=40,col='dark grey')+
     geom_vline(xintercept = end_current_period,col="blue",linetype=2)
  #########################################################################
   })
...
renderPlot({
  ...
  p+geom_line(stat="identity",size=1.3)+facet_wrap(~Area, ncol=2)+ylab('Spawning Biomass (x1000 t)')
  ##########################################################################
...
renderPlot({
  ...
  p+geom_line(stat="identity",size=1.3)+ylab('Spawning Biomass (x1000 t)')
  # ##########################################################################
...
renderPlot({
  ...
  ####################### FFMSY (SUMMED OVER AREAS) ###########################
   p <- ggplot(FFMSYplot, aes(x=Year, y=F.Fmsy,colour=Model_run))
  p+geom_line(stat="identity",size=1.3)+ylab('F/Fmsy')
  ##########################################################################
})
```
...
renderPlot({
  ...
  p+geom_line(stat="identity",size=1.3)+facet_wrap(~Area, ncol=2)+ylab('Recruit 0 (x1000)')
  ##########################################################################
})
```
...
renderPlot({
  ...
  p <- ggplot(forR0, aes(x=Year, y=rec0,colour=Model_run))
  p+geom_line(stat="identity",size=1.3)+ylab('Recruit 0 (x1000)')
  ##########################################################################
})
```
...
renderPlot({
  ...
  forSize <- ddply(na.exclude(Sselplot), .(Fleet,Size,Model_run),
                 function(x) data.frame(x[x$Gender==1 & x$Factor=="Lsel" & x$Year==2015,]))
  p <- ggplot(forSize, aes(x=Size, y=sel,colour=Model_run))
p+geom_line(stat="identity",size=1.3)+facet_wrap(~Fleet, ncol=5, scales="free")#+
