...
dirWS <- "http://data.d4science.org/Y1NrS3pLQmNxTERueUlqUVo1cVVwKzF1NkQ1a1RaSWtHbWJQNStIS0N6Yz0"
...
selectizeInput(inputId='Model',label='Select from the species models currently available',
              choices=as.character(unique(run_parameters$Model)),selected='SWO',multiple = FALSE)
...
renderPlotly({
    ...
    p <- p+geom_bar(stat="identity")+facet_wrap(~Fleet, ncol=3, scales="free")+
      geom_line(aes(y=Exp,colour=Model_run),size=1.3)+ylab('Observations')+xlab('Length bin (cm)')#+ 
...
renderPlotly({
  ...
  p <- p+geom_line(stat="identity",size=1.3)+facet_wrap(~Area, ncol=2)+ylab('Recruit 0 (millions)')
  ggplotly(p)
  ##########################################################################
})
```
...
renderPlotly({
  ...
  p <- ggplot(forR0, aes(x=Year, y=rec0,colour=Model_run))
  p <- p+geom_line(stat="identity",size=1.3)+ylab('Recruit 0 (millions)')
  ggplotly(p)
  ##########################################################################
})
```
...
p <- p+geom_line(stat="identity",size=1.3)+facet_wrap(~Fleet, ncol=5, scales="free")+xlab('Length (cm)')+ylab('Proportion')#+
ggplotly(p)
  ##########################################################################
})
```
